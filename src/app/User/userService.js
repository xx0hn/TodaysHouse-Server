...
exports.createUser = async function (email, passWord, passWordCheck, nickName) {
  try {
    ...
    //확인 비밀번호 암호화
    const hashedPasswordCheck = await crypto.createHash('sha512').update(passWordCheck).digest('hex');
    //비밀번호 확인
...
exports.createSocialUser = async function (name, email, loginStatus) {
  try {
    ...
    const userIdResult = await userDao.insertSocialUserInfo(connection, insertSocialUserInfoParams);
    const userInfoRows = await userProvider.accountCheck(email);
    if (userInfoRows[0].status === 'INACTIVE') {
      return errResponse(baseResponse.SIGNIN_INACTIVE_ACCOUNT);
    } else if (userInfoRows[0].status === 'DELETED') {
...
exports.postScrapFolders = async function (userId, folderName, folderInfo) {
  try {
    ...
    if (folderNameRows.length > 0) return errResponse(baseResponse.REDUNDANT_FOLDER_NAME);
    const connection = await pool.getConnection(async conn => conn);
